...
auto glaze_test()
{
    ...
    std::cout << "CURRENT VALUES: " << buffer << std::endl;
...
auto jsonifier_test()
{
    ...
    for (size_t i = 0; i < iterations; ++i) {
        if (!parser.parseJson < jsonifier::parse_options{ .knownOrder = true , .minified = true } > (obj, buffer)) {
    ...
    std::cout << "CURRENT VALUES: " << buffer << std::endl;
    results r{ minified ? "Jsonifier (.minified)" : "Jsonifier", "https://github.com/realtimechris/jsonifier", iterations };
    r.json_roundtrip = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count() * 1e-6;
...
auto jsonifier_abc_test()
{
    ...
    for (size_t i = 0; i < iterations_abc; ++i) {
        if (!parser.parseJson < jsonifier::parse_options{ .knownOrder = false, .minified = true } > (obj, buffer)) {
            std::cout << "jsonifier error!\n";
            break;
        }
    }
...
auto boost_json_test()
{
    ...
    try {
        for (size_t i = 0; i < iterations; ++i) {
            ...
            obj = boost::json::object_to<obj_t>(jv);
...
void test0()
{
    ...
    results.emplace_back(jsonifier_test <true>());
